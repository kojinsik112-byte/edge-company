...
if "%~1"=="" (
  ...
  echo.
  echo   결과는 "바탕화면 \ 캡컷_편집결과" 폴더에 바로 생깁니다.
  echo.
...
REM 바탕화면 경로 구하기 (OneDrive 로 옮겨진 경우까지 정확히)
set "DESKTOP=%USERPROFILE%\Desktop"
for /f "usebackq delims=" %%D in (`powershell -NoProfile -Command "[Environment]::GetFolderPath('Desktop')"`) do set "DESKTOP=%%D"
set "OUTDIR=%DESKTOP%\캡컷_편집결과"
...
echo 편집 시작: %~1
echo 결과는 바탕화면의 "캡컷_편집결과" 폴더에 저장됩니다.
echo 영상 길이에 따라 시간이 걸립니다. 창을 닫지 마세요.
echo.
python -m autoedit.cli edit "%~1" -o "%OUTDIR%" -v %SRTOPT%
if errorlevel 1 (
  echo.
...
if not exist "%SIDESRT%" if exist "%OUTDIR%\%~n1.srt" (
  copy /y "%OUTDIR%\%~n1.srt" "%SIDESRT%" >nul
  echo.
  echo [자막 수정 안내]
...
echo.
echo ============================================
echo    완료! 결과가 바탕화면 "캡컷_편집결과" 폴더에 있습니다.
echo      - 완성 영상: _edited.mp4
echo      - 자막: .srt 파일
echo      - 숏츠: shorts 폴더
echo ============================================
explorer "%OUTDIR%"
