...
5. 끝! 잠시 기다리면 **바탕화면의 `캡컷_편집결과` 폴더**가 자동으로 열리며
   그 안에 **완성 영상 + 자막 + 숏츠**가 바로 나타납니다.
...
3. **자막** — 오디오를 16kHz WAV로 추출 → `faster-whisper` 로 인식 → SRT 작성(직접 수정 가능) → 영상 해상도에 맞춘 ASS로 변환해 `subtitles` 필터로 번인. ASS의 PlayRes를 영상 크기에 고정해 **자막 크기를 정확한 픽셀 단위로** 통제합니다.
