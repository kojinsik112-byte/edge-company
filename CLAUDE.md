...
## 조직 (33개 팀 = Claude Code 서브에이전트, `.claude/agents/`)
**🔍 리서치 본부**
...
13. **cgi_3d** — 3D·CGI (다각도·분해도·단면·360°)
14. **motion** — 모션그래픽·시네마그래프·GIF
15. **video** — 상세페이지/숏폼 영상 (캡컷 autoedit)

**🤖 AI 본부**
16. **prompt_engineer** — 모델별 이미지 프롬프트 최적화·라이브러리
17. **model_trainer** — 아크로 전용 LoRA 학습 (일관성 무기)
18. **ai_ops** — 파이프라인·어댑터·MCP·모델선택 관리 (엔지니어링 백본)
19. **innovation** — 신규 AI 모델/도구 상시 감시·도입

**🛡️ 품질·법무 본부**
20. **auditor** — 검수 (사실·표절·규격) ← 회장이 가장 강조
21. **legal** — 표시광고법·전안법(KC)·과대광고 전문
22. **visual_qa** — 픽셀·가독성·모바일·등록규격 최종 관문

**🚀 운영·성과 본부**
23. **operator** — 네이버 등록
24. **analytics** — 등록 후 성과분석·개선루프
25. **experiment_lab** — A/B 실험 설계·검증

**📊 전략·인텔리전스 본부**
26. **competitive_intel** — 경쟁 상시 모니터링 (신제품·가격·순위 변화)
27. **product_strategy** — 상품기획·포지셔닝 (미끼→수익 설계)
28. **pricing** — 가격전략 (미끼가격·번들·마진 시뮬레이션)
29. **brand_intel** — 자사 분석 (우리를 가장 잘 아는 팀)

**🔗 커머스·생태계 본부**
30. **ecosystem** — 크로스셀·번들 (실링팬→조명·스위치·커튼 통일)

**📦 경영·운영 본부**
31. **inventory** — 재고관리 (SKU·입출고·발주점, `business-ops/`)
32. **barcode** — 바코드·품번 식별 (Code128/EAN-13)
33. **supervisor** — 본부 감독·조율 (팀을 감독하는 팀)
