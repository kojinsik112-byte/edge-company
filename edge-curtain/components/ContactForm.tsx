...
export default function ContactForm() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", region: "울산", category: "커튼", message: "" });
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-line bg-surface p-6 text-left sm:p-8">
      ...
      <button type="submit" disabled={busy} className="mt-4 w-full rounded-lg bg-ink py-3.5 text-[15px] font-bold text-white transition hover:bg-[#1f1916] disabled:opacity-60">
        {busy ? "접수 중…" : "상담 신청하기"}
      </button>
