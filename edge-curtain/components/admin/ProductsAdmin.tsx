    ...
    <>
      <form onSubmit={add} className="mb-8 space-y-3 rounded-2xl border border-line bg-surface p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <input className={inp} placeholder="제품명 (예: 암막 커튼)" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} />
          <input className={inp} placeholder="분류 (예: 커튼 / 블라인드 / 전동커튼)" value={f.category} onChange={(e) => setF({ ...f, category: e.target.value })} />
        </div>
