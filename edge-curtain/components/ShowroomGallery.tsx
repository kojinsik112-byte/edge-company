        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4">
          {images.map((src, i) => (
            <div key={i} className="relative aspect-[4/3] overflow-hidden rounded-[20px] bg-surface">
              <Image src={src} alt={`엣지리브커튼 쇼룸 ${i + 1}`} fill sizes="(max-width:768px) 50vw, 380px" className="object-cover transition duration-500 hover:scale-[1.04]" />
            </div>
          ))}
          ...
          <Link href="/showroom" className="group flex aspect-[4/3] flex-col items-center justify-center gap-3.5 rounded-[20px] bg-[#cbd5b6] p-6 text-center text-ink transition hover:brightness-[0.97]">
            ...
            <span className="text-[12.5px] text-ink/60">울산 커튼·블라인드 쇼룸</span>
          </Link>
        </div>
      </div>
    </section>
