...
export default async function Home() {
  const [settings, curtainCases, blindCases, motorCases, reviews, faqs, products, videos] = await Promise.all([
    getSettings(),
    getCases({ category: "커튼", limit: 6 }),
    getCases({ category: "블라인드", limit: 6 }),
    getCases({ category: "전동커튼", limit: 6 }),
    getReviews(12),
    getFaqs(),
    getProducts(),
    getYoutube(),
  ...
  return (
    <>
      ...
      <section className="relative h-[calc(100svh-72px)] min-h-[560px] overflow-hidden bg-warm">
        {/* PC(가로) */}
        <Image src={hero.image} alt={`${hero.subline} — 엣지리브커튼`} fill priority sizes="100vw" className="hidden object-cover md:block" />
        {/* 모바일(세로) — 없으면 PC 이미지 */}
        <Image src={hero.imageMobile || hero.image} alt={`${hero.subline} — 엣지리브커튼`} fill priority sizes="100vw" className="object-cover md:hidden" />
      ...
      <CategoryCases title="커튼 시공사례" desc="공간 무드를 바꾸는 맞춤 커튼" cases={curtainCases} moreHref={moreHref("커튼")} bg="bg-bg" />
      <CategoryCases title="블라인드 시공사례" desc="깔끔한 라인과 빛 조절" cases={blindCases} moreHref={moreHref("블라인드")} bg="bg-surface" />
      <CategoryCases title="전동커튼 시공사례" desc="버튼 하나로 여닫는 편리함" cases={motorCases} moreHref={moreHref("전동커튼")} bg="bg-bg" />
