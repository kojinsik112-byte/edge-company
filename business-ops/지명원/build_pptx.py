...
# ============ 유상옵션 ============
s=slide(); header(s,"13","유상옵션","Paid Options")
txt(s,0.7,1.82,12,0.35,[[("입주 단지 공동구매·유상옵션으로 함께 제공합니다.",12,RGBColor(0x3A,0x44,0x56),False)]])
opts=[("❄️","에어컨","시스템·벽걸이 에어컨 설치·공동구매"),
("🚪","중문","현관 중문 — 단열·소음·먼지 차단, 디자인 선택"),
("🎨","친환경 도료","무독성 친환경 도장 — 새집증후군 저감"),
("🌬️","환기 시스템","전열교환 환기로 실내 공기질 상시 관리"),
("♻️","음식물 처리기","싱크대 음식물 분쇄·건조 처리"),
("➕","기타 옵션","입주 단지 맞춤 유상옵션 상시 추가 구성")]
ocw=3.8; och=2.0; ogx=0.3; ogy=0.32; ox0=0.7; oy0=2.35
for i,(ic,t,d) in enumerate(opts):
    r=i//3; c=i%3
    cx=ox0+c*(ocw+ogx); cy=oy0+r*(och+ogy)
    rect(s,cx,cy,ocw,och,fill=WHITE,line=LINE,lw=1,round_=True)
    rect(s,cx+0.26,cy+0.26,0.7,0.7,fill=ICE,round_=True)
    txt(s,cx+0.26,cy+0.26,0.7,0.7,[[(ic,21,BLUE,False)]],align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    txt(s,cx+1.14,cy+0.36,ocw-1.34,0.5,[[(t,15,INK,True)]])
    txt(s,cx+1.14,cy+0.92,ocw-1.34,0.95,[[(d,10.8,SUB,False)]],sp=1.15)
note(s,0.7,6.9,12.0,"정확 품목·시공범위·가격은 단지/회장 확정 후 구성 (가격 미표기).",h=0.4)

# ============ S13 스마트홈 앱 ============
s=slide(); header(s,"14","ACRO 스마트홈 — 국내 최초 통합 앱","Smart-Home System")
...
# ============ S13 강점·협력 ============
s=slide(); header(s,"15","왜 엣지컴퍼니인가","Why Edge Company")
...
# ============ S14 연락처 ============
s=slide(); header(s,"16","연락처","Contact")
