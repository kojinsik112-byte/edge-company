...
| **한국형 조판·폰트·인포그래픽 마감** | `finish` | **망고보드** (AI 상세페이지 + Chat망고) | 자체 HTML 엔진 |

### 망고보드 = 조판 마감 100점 라인업

망고보드는 한국형 상세페이지 템플릿·폰트·인포그래픽이 가장 강하고, 2026년 **AI 상세페이지 제작
서비스**까지 출시됐습니다. 단, 공개 API가 없어 완전 무인은 안 됩니다. 그래서 우리는 역할을 나눕니다.

- **이미지 생성(Imagen/Gemini 등)** = 우리 AI가 최고화질로 뽑는다.
- **한국형 조판·폰트·인포그래픽** = 망고보드가 마감한다.

파이프라인은 빌드할 때마다 `output/<product>/mangoboard/` 에 **망고보드 작업 패키지**를 만듭니다:
`작업지시서.md`(슬라이드별 카피·이미지·인포그래픽 수치) + `manifest.json` + `assets/`(AI 생성 이미지).
이걸 망고보드 템플릿(또는 AI 상세페이지/ Chat망고)에 그대로 꽂으면 완성됩니다.

> 자체 HTML 엔진(Jinja2+Playwright)은 망고보드 조립 전 **빠른 미리보기**용으로 남겨둡니다.
