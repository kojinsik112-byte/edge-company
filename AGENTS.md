...
## 🔴🔴🔴 제품 이미지 절대 규칙 (회장 반복 분노 — 절대 잊지 말 것)
- **AI로 아크로 제품(실링팬)을 절대 생성·지어내지 말 것.** Flux·Gemini·Ideogram으로 만든 팬은 우리 실제 제품과 생김새가 다르다 = 가짜 = 회장 분노.
- **제품 비주얼은 무조건 회장 실제 자산** `C:\Users\win11\Desktop\아크로 사진\`(ChatGPT로 만든 NEW SLIM 흰 슬림팬·앱 인포그래픽 등)을 **그대로 쓰거나 크롭**해서 사용.
- AI는 **제품이 안 나오는 배경/공간/소품에만** 보조로. 제품 본체는 절대 AI 생성 금지.
- 한글 텍스트는 HTML/Pillow(Pretendard·맑은고딕)로 얹기(AI 한글 깨짐).
- 회장 자산이 이미 프로급 마케팅 이미지다 — 흉내내지 말고 그걸 활용/보강하라.
