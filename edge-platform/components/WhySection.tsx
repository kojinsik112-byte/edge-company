      ...
      <div className="reveal mx-auto max-w-[1320px] px-6">
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-14">
          ...
          <div>
            ...
            <h2 className="mt-3 text-[26px] font-display font-extrabold leading-snug text-ink md:text-[32px]">{company.heading}</h2>
            {company.lead && <p className="mt-4 whitespace-pre-line text-[15.5px] leading-relaxed text-muted">{company.lead}</p>}
            {features.length > 0 && (
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                {features.map((f, i) => (
                  <div key={i} className="rounded-2xl border border-line bg-surface p-6 transition duration-300 hover:-translate-y-1 hover:shadow-[0_22px_45px_-24px_rgba(47,37,28,0.3)]">
