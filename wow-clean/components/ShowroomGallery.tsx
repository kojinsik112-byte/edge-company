        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4">
          {images.map((src, i) => (
            <div key={i} className="relative aspect-[4/3] overflow-hidden rounded-[20px] bg-surface">
              <Image src={src} alt={`와우클린 시공 현장 ${i + 1}`} fill sizes="(max-width:768px) 50vw, 380px" className="object-cover transition duration-500 hover:scale-[1.04]" />
            </div>
          ))}

          {/* 마지막 칸: 시공현장 안내 CTA 타일 (라이트 블루) */}
          <Link href="/showroom" className="group flex aspect-[4/3] flex-col items-center justify-center gap-3.5 rounded-[20px] bg-[#cfe3f7] p-6 text-center text-ink transition hover:brightness-[0.97]">
            ...
            <span className="text-[18px] font-extrabold">시공 현장 더 보기</span>
            <span className="text-[12.5px] text-ink/60">전국 24시간 출동 서비스</span>
          </Link>
        </div>
      </div>
    </section>
