...
export default function SettingsForm({ initial }: { initial: Settings }) {
  ...
  return (
    <div className="space-y-10">
      ...
      {/* 회사소개 (왜 와우클린) */}
      <Section title="회사소개 (왜 와우클린일까요?)" onSave={() => save("company")} saving={saving === "company"}>
        <ImageField label="대표 이미지 (좌측 비주얼)" hint="1200 × 1500px (4:5 세로)" url={s.company.image} onPick={(f) => pick(f, (url) => patch("company", { image: url }))} />
        <Grid>
          <Field label="상단 영문"><Inp value={s.company.eyebrow} onChange={(v) => patch("company", { eyebrow: v })} placeholder="Why Wow Clean" /></Field>
          <Field label="큰 제목"><Inp value={s.company.heading} onChange={(v) => patch("company", { heading: v })} placeholder="왜 와우클린일까요?" /></Field>
        </Grid>
      ...
      {/* 시뮬레이터 (와우클린 미사용 — 기본 OFF) */}
      <Section title="시뮬레이터 (미사용 · 기본 OFF)" onSave={() => save("simulator")} saving={saving === "simulator"}>
        ...
        <div className="space-y-3">
          {s.shorts.items.map((it, i) => (
            <div key={i} className="relative space-y-2 rounded-xl border border-line p-3">
              ...
              <Inp value={it.title} onChange={(v) => setShort(i, { title: v })} placeholder="제목 (예: 하수구 뚫음 비포애프터)" />
      ...
      {/* 시공 현장 */}
      <Section title="시공 현장 안내" onSave={() => save("showroom")} saving={saving === "showroom"}>
        ...
        <Field label="접수 안내"><Inp value={s.showroom.hours} onChange={(v) => patch("showroom", { hours: v })} /></Field>
        <MultiImage
          label="시공 현장 갤러리 (4~6장)"
        ...
        <div className="space-y-3">
          {CATEGORIES.map((c) => (
            <div key={c} className="space-y-2 rounded-xl border border-line p-3">
              ...
              <Inp value={s.categorySections[c]?.title ?? ""} onChange={(v) => setCatSec(c, { title: v })} placeholder="섹션 제목 (예: 하수구막힘 시공사례)" />
      ...
      {/* 사업자등록증·증빙 */}
      <Section title="사업자등록증·증빙" onSave={() => save("license")} saving={saving === "license"}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {s.license.items.map((it, i) => (
            <div key={i} className="space-y-2 rounded-xl border border-line p-3">
              ...
              <Inp value={it.caption} onChange={(v) => setLicenseItem(i, { caption: v })} placeholder="등록증 이름 (예: 사업자등록증)" />
            </div>
          ))}
        </div>
      </Section>
...
const LABEL: Record<string, string> = {
  ...
  simulator: "시뮬레이터", process: "시공 절차", shorts: "숏츠",
  showroom: "시공 현장", categories: "카테고리 이미지", categorySections: "카테고리 이름", seo: "SEO",
  license: "사업자등록증", branches: "출동 지역", partners: "협력사",
};
