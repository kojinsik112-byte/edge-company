...
export default function CaseForm({ initial }: { initial?: CaseRow }) {
  ...
  const [region, setRegion] = useState<Region>(initial?.region ?? "서울");
  const [category, setCategory] = useState<Category>(initial?.category ?? "하수구막힘");
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div>
        ...
        <input className={input} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="예) 강서구 화곡동 빌라 하수구막힘 시공" />
      </div>
      ...
      <div>
        ...
        <input className={input} value={tagsText} onChange={(e) => setTagsText(e.target.value)} placeholder="예) 저천장, 슬림팬, 우물천장" />
