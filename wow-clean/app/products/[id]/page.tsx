...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${p.name} — 와우클린 ${p.category}`,
    description: (p.body || "").slice(0, 120) || `와우클린 ${p.category} ${p.name}`,
  ...
  return (
    <article className="mx-auto max-w-[960px] px-5 py-10 md:py-14">
      <nav className="text-[12.5px] text-muted">
        <Link href="/products" className="hover:text-navy">서비스 안내</Link>
      ...
      <div className="mt-4 grid gap-8 md:grid-cols-2">
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="mt-7 flex gap-3">
            ...
            <Link href="/contact" className="rounded-lg border border-gold bg-surface px-6 py-3.5 text-[15px] font-bold text-ink transition hover:bg-bg">온라인 접수</Link>
          </div>
        </div>
      </div>
